  ...
  useEffect(() => {
    if (editorRef.current) {
      ...
      if (textboxElement) {
        ...
        textboxElement.addEventListener("touchstart", (e) => {
          if (editable && !localEditable) {
            setLocalEditable(true);
            // Add a small timeout before focusing to avoid weird iOS behaviors
            setTimeout(() => {
              view?.focus();
            }, 100);
          }
        ...
        // Prevent pinch zoom inside editor on mobile
        textboxElement.addEventListener("gesturestart", (e) => {
          e.preventDefault();
        });

        // Add better touch feedback
        textboxElement.addEventListener("touchend", () => {
          if (editable) {
            const el = textboxElement as HTMLElement;
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [editorRef.current, editable, view, localEditable]);
